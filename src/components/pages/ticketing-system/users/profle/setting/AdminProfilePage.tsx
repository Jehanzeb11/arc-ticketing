...
export default function AdminProfilePage ({ title }) {
  ...
  const { fetchAdminProfile, updateUser, callApi }: any = useApiStore()
  ...
  const profileData = users?.find(user => user.user_email === loggedInEmail)

  // Populate form fields when profileData is available
  React.useEffect(() => {
    if (profileData) {
      setName(profileData.user_name || '')
      setEmail(profileData.user_email || '')
      setPhoneNumber(profileData.user_phone || '')
      setRole(profileData.user_role || '')
  ...
  const updateMutation = useMutation({
    mutationFn: updatedData =>
      callApi(updateUser, profileData?.id, updatedData),
    onSuccess: () => {
